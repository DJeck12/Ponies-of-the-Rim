            ...
            foreach (Pawn item in p.MapHeld.mapPawns.FreeColonistsSpawned)
            {
                if (item == p)
                    continue;
                                ...
                                if (item.story?.traits == null)
                    continue;

                if (item.story.traits.HasTrait(Pony_DefOf.Pony_Joyous) && !item.HostileTo(p))
                {
                    return ThoughtState.ActiveWithReason(
                        def.stages[0].description.Formatted(item.Named("PAWN"))
                    );
                }
            }
